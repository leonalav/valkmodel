...
def test_config_rejects_invalid_latent_branching_fields():
    ...
    with pytest.raises(ValueError, match="num_branches"):
        ValkModelConfig(use_latent_state=True, use_latent_branching=True, num_branches=1)
    with pytest.raises(ValueError, match="branch_value_temperature"):
        ValkModelConfig(use_latent_state=True, use_latent_branching=True, branch_value_temperature=0.0)
    with pytest.raises(ValueError, match="branch_selection_mode"):
        ValkModelConfig(use_latent_state=True, use_latent_branching=True, branch_selection_mode="sample")
    with pytest.raises(ValueError, match="latent_branching_layers"):
        ValkModelConfig(use_latent_state=True, use_latent_branching=True, latent_branching_layers=[99])
...
def test_parameter_estimate_counts_each_latent_state_layer_exactly():
    ...
    latent_per_layer = (
        ...
        + (config.hidden_size + config.latent_state_dim) * config.latent_state_dim
